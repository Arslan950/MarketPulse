...
const ChartTooltipContent = ({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label }) => {
    ...
    return (<div className={cn("grid min-w-32 items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl", className)}>
      ...
      <div className="grid gap-1.5">
        {payload.
            ...
            map((item, index) => <div key={index} className={cn("flex w-full flex-wrap items-stretch gap-2", indicator === "dot" && "items-center")}>
              {!hideIndicator &&
...
const ChartLegendContent = ({ className, hideIcon = false, payload, verticalAlign = "bottom" }) => {
    if (!payload?.length)
        return null;
    return (<div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.
            filter((item) => item.type !== "none").
            map((item, i) => <div key={i} className="flex items-center gap-1.5">
